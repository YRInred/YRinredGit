# YRinredGit
记录一下三方自己比较习惯的控件 各个类型只会保留一个 倒不是说其他的不好用 只是觉得技术最好能化繁为简才能窥其大道 根据使用习惯不定时更新
## 列表adapter工具
[BaseRecyclerViewAdapterHelper](http://www.jianshu.com/p/b343fcff51b0)
## 刷新列表控件
[TwinklingRefreshLayout](https://github.com/lcodecorex/TwinklingRefreshLayout/blob/master/README_CN.md)
[EasyRefreshLayout(配合使用BaseRecyclerViewAdapterHelper)](https://github.com/anzaizai/EasyRefreshLayout)
## 网络线程相关
[Rxjava2](https://github.com/ReactiveX/RxJava)
[RxAndroid2](https://github.com/ReactiveX/RxAndroid)
[Rxbinding3](https://github.com/JakeWharton/RxBinding)
[okhttp3](https://github.com/square/okhttp)
[okhttplog](https://github.com/square/okhttp/tree/master/okhttp-logging-interceptor)
[retrofit2](https://github.com/square/retrofit)
[retrofit2adapter](https://github.com/square/retrofit/tree/master/retrofit-adapters)
[retrofit2gson](https://github.com/square/retrofit/tree/master/retrofit-converters/gson)
## 权限处理
[Rxpermissisons](https://github.com/tbruyelle/RxPermissions)
## 图片加载
[Glide](https://github.com/bumptech/glide)
[glideCompiler](https://github.com/bumptech/glide)
[okhttpintegration](https://github.com/bumptech/glide/tree/master/integration/okhttp3)
## 图片定制(不规则图片，毛玻璃等等效果)
[glide-transformations(配合glide使用)](https://github.com/wasabeef/glide-transformations)
## 图片压缩算法
[Luban](https://github.com/Curzibn/Luban)
## 图片放大
[PhotoView](https://github.com/chrisbanes/PhotoView)
## 访微信图片拖动关闭
[DragPhotoView](https://github.com/githubwing/DragPhotoView)
## 图片选择器(这块目前还没有找到完美兼容所有需求的控件)
[rxgalleryfinal](https://github.com/FinalTeam/RxGalleryFinal)
[RZAlbum](https://github.com/ray00178/RZAlbum-Java)
## 注解
[ButterKnife](https://github.com/JakeWharton/butterknife)
[butterknifeCompiler](https://github.com/JakeWharton/butterknife)
## 城市选择控件
[citypicker](https://github.com/crazyandcoder/citypicker)
## bug监控工具
[bugly](https://bugly.qq.com/v2/)
## 常用工具类
[util](https://github.com/Blankj/AndroidUtilCode)
## 加载框
[KProgressHUD](https://github.com/Kaopiz/KProgressHUD)
## 指纹判断
[fingerprint](https://github.com/uccmawei/FingerprintIdentify)
## 图表工具
[MPAndroidChart](https://github.com/PhilJay/MPAndroidChart)
## 减少shape资源文件布局
[FlycoRoundView](https://github.com/H07000223/FlycoRoundView)
## 简化开发时间共用布局(比如个人信息界面)
[LovelyView](https://github.com/samlss/LovelyView)
## 通过标签生成shape,无需再写shape
[BackgroundLibrary](https://github.com/JavaNoober/BackgroundLibrary)
## 屏幕适配方案
[AndroidAutoSize](https://github.com/JessYanCoding/AndroidAutoSize)
## 日历控件
[CalenderView](https://github.com/huanghaibin-dev/CalendarView)
## 图片轮播
[banner](https://github.com/youth5201314/banner)
## 仿美团分页按钮控件
[FJMtSortButton](https://github.com/FJ917/FJMtSortButton)
## 扁平化dialog
[material-dialogs](https://github.com/afollestad/material-dialogs)
## 页面滑动指示器
[MagicIndicator](https://github.com/hackware1993/MagicIndicator)
## 页面状态切换view
[MultiStateView](https://github.com/Kennyc1012/MultiStateView)
## 状态栏工具
[StatusBarUtil](https://github.com/laobie/StatusBarUtil)
## 手势解锁
[PatternLockView](https://github.com/aritraroy/PatternLockView)
## 多条件筛选菜单
[DropDownMenu](https://github.com/dongjunkun/DropDownMenu)
